...
> In May 2025, Google announced that location data would no longer be stored on their servers. It's now stored on the device and only sent to Google in an encrypted form. 
> This means that the Google Takeout export doesn't contain location data anymore.
> It's still possible to export the data from the device, but it only contains the raw location data, not the "semantic" one that used to be available and is used here.
> Therefore, the parts about the "Semantic Location History" are now obsolete forever. The rest of the article is still relevant, though.
{: .prompt-warning }
